...
if(isset($_SESSION['teamNo']) && isset($_SESSION['teamName'] ) )  {
	...
	header("Location:index.php"); //redirect to index page if user not logged-in used when user logged in.

if(!$login)	
	header("Location:index.php"); //redirect to index page if user not logged-in used when user logged in.

else  {
	include_once "views/common/headerLogin.php" ; //used when user not logged in.
}
